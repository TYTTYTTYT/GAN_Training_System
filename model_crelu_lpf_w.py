...
class Complex_Fully_Connected_Linear_Discriminator_LPF(nn.Module):
    ...
    def forward(self, x):
        ...
        xrn = F.leaky_relu(self.linear1r(xr), 0.1) - F.leaky_relu(self.linear1i(xi), 0.1)
        xin = F.leaky_relu(self.linear1r(xi), 0.1) + F.leaky_relu(self.linear1i(xr), 0.1)
        xr = xrn
        xi = xin
...
class Crelu_Fully_Connected_WGAN_LPF_W(nn.Module):
    def __init__(self, dimension):
        super(Crelu_Fully_Connected_WGAN_LPF_W, self).__init__()
