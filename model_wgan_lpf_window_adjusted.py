...
class Complex_Fully_Connected_Linear_Discriminator_LPF(nn.Module):
    def __init__(self, dimension):
        ...
        self.linear2 = nn.Linear(self.n_in * 4, self.n_in * 4)
        self.linear3 = nn.Linear(self.n_in * 4, self.n_in * 4)
        self.linear4 = nn.Linear(self.n_in * 4, self.n_in * 4)
        self.linear5 = nn.Linear(self.n_in * 4, self.n_in * 4)
        self.linear6 = nn.Linear(self.n_in * 4, self.n_in * 4)
        self.linear7 = nn.Linear(self.n_in * 4, self.n_in * 4)
        self.linear8 = nn.Linear(self.n_in * 4, self.n_in * 1)
    ...
    def forward(self, x):
        ...
        x = F.leaky_relu(self.linear4(x), 0.1)
        x = F.leaky_relu(self.linear5(x), 0.1)
        x = F.leaky_relu(self.linear6(x), 0.1)
        x = F.leaky_relu(self.linear7(x), 0.1)
        x = self.linear8(x)
...
class Complex_Fully_Connected_WGAN_LPF_W_Adjusted(nn.Module):
    def __init__(self, dimension):
        super(Complex_Fully_Connected_WGAN_LPF_W_Adjusted, self).__init__()
